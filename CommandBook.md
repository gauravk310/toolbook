...
Saves to `~/Downloads` by default, or to the path you specify.

```bash
toolbook media yt-vdo <URL> [PATH]
```

| Argument | Required | Description |
|----------|----------|-------------|
| `URL` | Yes | YouTube video URL to download |
| `PATH` | No | Destination folder (default: `~/Downloads`). Pass `.` to download into the current directory |
...
toolbook media yt-vdo https://youtu.be/dQw4w9WgXcQ C:\Users\me\Videos
...
result = VideoDL.YT("https://youtu.be/dQw4w9WgXcQ", "/path/to/folder")

# Download to ~/Downloads (path=None)
result = VideoDL.YT("https://youtu.be/dQw4w9WgXcQ")
```
